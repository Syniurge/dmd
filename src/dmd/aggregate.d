...
extern (C++) abstract class AggregateDeclaration : ScopeDsymbol
{
    ...
    Scope* semScope; // FWDREF FIXME temporary field to make _scope/sc available to determineSymtab during a dsymbolSemantic(AggregateDeclaration) called
                    //  _scope needs to stop being used as check for recursion or semantic state
    ...
    {
        if (!sc)
            sc = _scope ? _scope : semScope; // FWDREF FIXME
